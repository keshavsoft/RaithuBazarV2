...
let LocalFuncFor1 = ({ value, key }) => {
    ...
    const jVarLocalLeftCol = key.split(".").length >= 4 ? `col-${key.split(".")[3]}` : "col";
    const jVarLocalRightCol = key.split(".").length >= 5 ? `col-${key.split(".")[4]}` : "col";

    jFLocalForImageTagAnd5dot7({
        inItemName: key.split(".")[0],
        inRate: jVarLocalRate1,
        inImageId: `${key.split(".")[1]}.${key.split(".")[2]}`,
        inLeftCol: jVarLocalLeftCol,
        inRightCol: jVarLocalRightCol
    });
...
const jFLocalForImageTagAnd5dot7 = ({ inRate, inImageId, inItemName, inLeftCol, inRightCol }) => {
    if ("content" in document.createElement("template")) {
        const tbody = document.querySelector("#RowContainerId");
        const template = document.querySelector("#TemplateForImageAndSingleColumnDynamic");
        ...
        jVarLocalItemRateFind.innerHTML = inRate;
        debugger;
        const jVarLocalFindLeftCol = clone.querySelector(".ImageClass");
        jVarLocalFindLeftCol.classList.add(inLeftCol);

        const jVarLocalFindRightCol = clone.querySelector(".ContentClass");
        jVarLocalFindRightCol.classList.add(inRightCol);
